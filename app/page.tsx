...
export default function Home(){
  ...
  return <main className={`site${rtl?" rtl":""}${isAndroid?" android":""}`} dir={rtl?"rtl":"ltr"}>
    ...
    <section className="firstAxesCase" id="first-axes-case">
      ...
      <div className="firstAxesPageEvidence"><div className="firstAxesSectionHead"><p>{rtl?"تصميمات فعلية مختارة من فترتي":"SELECTED STATIC DESIGNS FROM MY TENURE"}</p><span>{rtl?"دوري: الكتابة + الفكرة + الاتجاه":"MY ROLE: WRITING · CONCEPT · DIRECTION"}</span></div><div className="axesFeedGrid">{firstAxesDesigns.map((design,i)=><a href={design.href} target="_blank" rel="noreferrer" key={design.href} aria-label={rtl?design.ar:design.en}><img src={design.image} alt={rtl?design.ar:design.en} loading="lazy" decoding="async"/><span>0{i+1}</span><i>↗</i></a>)}</div><p>{rtl?<>دي تصميمات ثابتة أصلية من الفترة التي كنت مسؤولًا فيها عن الكتابة والفكرة والاتجاه، وليست أغلفة ريلز. التنفيذ البصري للفريق، وأي أعمال منشورة بعد <a href="https://www.instagram.com/p/DYtxU-BIg4v/" target="_blank" rel="noreferrer">هذا المنشور ↗</a> لا أنسبها لنفسي.</>:<>These are original static designs—not reel covers—from the period in which I owned the writing, concept, and direction. Visual execution belonged to the team; I do not claim work published after <a href="https://www.instagram.com/p/DYtxU-BIg4v/" target="_blank" rel="noreferrer">this post ↗</a>.</>}</p></div>
