...
export default function Home(){
  ...
  return <main className={rtl?"site rtl":"site"} dir={rtl?"rtl":"ltr"}>
    <nav className="newNav"><span className="navSpacer" aria-hidden="true"/><div className="navCenter">{t.nav.map(([href,label])=><a key={href} href={href}>{label}</a>)}</div><div className="navActions"><button className="langSwitch" onClick={()=>setLang(rtl?"en":"ar")} aria-label={rtl?"Switch to English":"التبديل للعربية"}>{rtl?"EN":"ع"}</button><a className="contactPill" href={whatsappUrl} target="_blank" rel="noreferrer">{t.contact} <span>↗</span></a></div></nav>
    ...
    <section className="mariemCase" id="mariem-case">
      ...
      <div className="salesRun"><div className="salesRunHead"><p>{rtl?"إجمالي مبيعات البراند أثناء فترة إدارة المحتوى":"TOTAL BRAND SALES DURING THE CONTENT TENURE"}</p><b>≈ 1.57M EGP</b></div><div className="salesBars">{[{m:"M1",v:"70K",h:"12%"},{m:"M2",v:"400K",h:"67%"},{m:"M3",v:"600K",h:"100%"},{m:"M4",v:"500K",h:"83%"}].map(x=><article key={x.m}><div><span style={{height:x.h}} /></div><small>{x.m}</small><b>{x.v}</b></article>)}</div><p className="salesNote">{rtl?"هذه أرقام إجمالي مبيعات البراند وليست مبيعات منسوبة للمحتوى وحده. ساهم المحتوى ضمن منظومة شملت المنتج والميديا باير وفريق التنفيذ.":"These are total brand sales, not sales attributed to content alone. Content contributed within a wider system including product, media buying, and the production team."}</p></div>
    ...
    <section className="morehCase" id="moreh-case">
      ...
      <div className="morehStats"><article><b>77K</b><span>{rtl?"جنيه مبيعات":"EGP SALES"}</span></article><article><b>7K</b><span>{rtl?"جنيه إنفاق إعلاني":"EGP AD SPEND"}</span></article><article className="morehAccent"><b>≈11×</b><span>{rtl?"عائد على الإنفاق":"RETURN ON AD SPEND"}</span></article><article><b>10</b><span>{rtl?"أيام للنتيجة":"DAYS TO RESULT"}</span></article></div>
      ...
      <div className="morehEvidence"><figure><img src="/moreh/results.png" alt={rtl?"لقطة من نتائج حملة مريح":"Moreh campaign results dashboard"}/></figure><div><small>{rtl?"لقطة أداء إضافية":"ADDITIONAL PERFORMANCE SNAPSHOT"}</small><h3>7.56× ROAS</h3><ul><li><b>27</b>{rtl?" عملية شراء":" purchases"}</li><li><b>139.57 EGP</b>{rtl?" تكلفة الشراء":" cost per purchase"}</li><li><b>5.14%</b> CTR</li><li><b>24,469</b>{rtl?" وصول":" reach"}</li></ul><p>{rtl?"هذه اللقطة تعرض نطاقًا مختلفًا أو جزءًا من الحملة عن ملخص أول 10 أيام؛ لذلك تُعرض كدليل أداء منفصل.":"This dashboard covers a different range or subset from the first 10-day summary, so it is shown as separate supporting evidence."}</p></div></div>
